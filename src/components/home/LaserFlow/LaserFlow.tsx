...
export function LaserFlow({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      uniforms.iResolution.value.set(width * dpr, height * dpr, dpr);
    };

    const updateMouse = (clientX: number, clientY: number) => {
      const rect = renderer.domElement.getBoundingClientRect();
      const dpr = uniforms.iResolution.value.z;
      mouseTarget.set((clientX - rect.left) * dpr, (rect.height - (clientY - rect.top)) * dpr);
    };
    ...
    renderer.domElement.addEventListener('pointerdown', handlePointerMove, { passive: true });
    renderer.domElement.addEventListener('pointerenter', handlePointerMove, { passive: true });
    ...
    const render = (now: number) => {
      if (now - lastFrameTime < TARGET_INTERVAL) {
        if (pause.running) frameId = window.requestAnimationFrame(render);
        return;
      }
      lastFrameTime = now;
      timer.update();
      ...
      mouseSmooth.lerp(mouseTarget, 1 - Math.exp(-delta / 0.1));
    ...
    return () => {
      ...
      renderer.domElement.removeEventListener('pointerdown', handlePointerMove);
      renderer.domElement.removeEventListener('pointerenter', handlePointerMove);
  ...
  return <div ref={mountRef} className={`${styles.root} ${className}`.trim()} />;
}
